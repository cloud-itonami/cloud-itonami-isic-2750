(ns domappl.operation
  "DomApplOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (DomApplAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Domestic Appliance Plant
...
(defn build
  "Compiles a DomApplOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `domappl.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; DomApplAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
